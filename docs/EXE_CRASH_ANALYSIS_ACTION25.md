...
## 6. #31 테스트 결과 (2026-03-09)

| 항목 | 결과 |
|------|------|
| **EXE 빌드** | ✓ 성공, 크래시 해결 |
| **P/R** | ✗ 매우 낮음 (~0.1), Recall 99.9%·Precision 99% 미달 |
| **conf=0.01** | Upper:0, Lower:0 (40핀 입력에 0개 탐지) |
| **추정 원인** | ROI 오수신 또는 masked 십자형(+) 마커 미인식 |

**후속 조치**: annotation.py 십자형 마커 인식 개선, 결과 경로 날짜시간별 정리 (ROADMAP 10.22)

---

## 7. 사전조치 완료 (2025-03-05)

| 항목 | 내용 |
|------|------|
| **ROI 전달 경로 검증** | docs/ROI_PATH_VERIFICATION.md, tools_scripts/verify_roi_path.py |
| **십자형 마커 옵션** | generate_pin_test_data.py --cross-markers |
| **Debug exe 제외** | 크래시 해결됨, pin_detection_gui_debug.exe 빌드 제거 |
