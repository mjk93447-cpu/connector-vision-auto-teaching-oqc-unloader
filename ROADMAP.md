...
### 10.25 Action #33 EXE·ROI Editor 개선 (2026-03-10)

**배경**: 아티팩트 #33 EXE 테스트 결과. 상세: `docs/EXE_ACTION33_ISSUES.md`

| # | 증상 | 대응 |
|---|------|------|
| 1 | Train epoch 로그 타이밍에서 크래시 | graph poll _draw_graph try/except, EXE matplotlib 방어 |
| 2 | 브러쉬로 핀 지정 어려움 | 스퀘어 도구: 작은 사각형 클릭으로 핀마다 개별 지정 |
| 3 | 초록 마커가 YOLO 학습 혼동 | 타겟 마스킹을 빨간색(255,0,0)으로 변경 |
| 4 | 잘못 지정 마스킹 지우기 없음 | Erase 모드 추가 |
| 5 | ROI·타겟 마스킹 YOLO 최우선 | annotation RED 우선 추출, dataset/inference 연동 |
| 6 | "Select folder first" 반복 | 검증 순서·메시지 명확화 |

**합성 테스트 데이터 경로** (검증 시 사용):

| 경로 | 용도 |
|------|------|
| `test_data/pin_synthetic/train/unmasked`, `.../masked` | 기본 (640×480, 40쌍) |
| `test_data/pin_large_factory/unmasked`, `.../masked` | 대용량 고화질 (5000×4000, 10쌍) |

생성: `python tools_scripts/generate_pin_test_data.py` (기본) 또는 `--output-dir test_data/pin_large_factory --large-factory --large-factory-n 10`

**EXE 테스트 시 "Select folder first" 방지**:
1. EXE를 **프로젝트 루트**에서 실행 (test_data와 같은 디렉터리)
2. Train 탭에서 **"Load test data"** 클릭 → unmasked/masked/output 자동 채움
3. 또는 EXE를 test_data가 있는 폴더와 같은 위치에 두고 실행
