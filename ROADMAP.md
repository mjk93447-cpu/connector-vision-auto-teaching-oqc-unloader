...
| **예상시간 부정확** | imgsz 5496 등 대형 입력 시 (imgsz/640)² 비례로 급증, CPU 전용·실측 미반영 | imgsz 상한(1280) 및 경고, CPU 보정계수, 보수적 추정 |
| **학습 미완료** | 대형 imgsz 시 메모리·시간 폭증, workers=8이 Windows multiprocessing 이슈 | imgsz 기본 640·최대 1280, workers 기본 4 |
...
| **imgsz 제한** | GUI Spinbox 최대 1280, 초과 입력 시 경고/클램프 | 대형 입력으로 인한 지연 방지 |
| **예상시간 공식** | CPU 보정(×3~5), imgsz² 스케일, 보수적 표시 | 사용자 기대에 부합 |
| **workers 기본값** | Windows에서 4 이하 권장 (spawn 오버헤드) | 안정적 학습 |
...
**배경**: 데이터 선택 시 이미지 크기는 인식되나, imgsz·epochs 등 최적 파라미터 자동 추천 기능 없음.

| 전략 | 설명 | 목표 |
|------|------|------|
| **빠른 스캔** | 샘플 3~5장만 분석, 1초 이내 완료 | UI 반응성 |
| **이미지 크기** | max(W,H) > 2000 → ROI 사용, cropped ~640px → imgsz 640 | 대형 이미지 |
| **핀 크기** | masked에서 bbox 추출, avg w×h < 120px² → 소형 핀 → imgsz 1280 | 소형 객체 정밀도 |
| **데이터량** | n < 15 → epochs 150 권장, n ≥ 20 → 100 | 과적합/과소적합 방지 |
| **자동 적용** | "Apply suggested" 버튼 또는 데이터 선택 시 자동 적용 옵션 | 사용자 편의 |

| 작업 | 설명 | 목표 |
|------|------|------|
| **analyze_dataset_for_training** | unmasked_dir, masked_dir → suggested imgsz, epochs, note | `pin_detection/dataset.py` |
| **GUI "Apply suggested"** | 추천값 적용 버튼, Dataset 라벨 옆 | 원클릭 적용 |
| **스캔 결과 표시** | "imgsz:640 (ROI crop), epochs:100" 등 요약 | 투명성 |
...
### 10.18 ROI 비활성화·Geometry Refinement 개선 (2026-03-06) — 완료
...
| Phase | 작업 | 목표 | 상태 |
|-------|------|------|------|
| **A** | ROI 비활성화 | 학습·추론 모두 전체 이미지 사용 | ✓ |
| **B** | epochs=3 | 학습 속도 극대화 (~2분 이내) | ✓ ~1.8 min |
| **C** | Geometry refinement 개선 | masked prior 활용 → 100% P/R | ✓ |
| **D** | 영역별 스캔 효율화 | 핀 클러스터 탐색 (필요 시) | 보류 |

**핵심**: masked_path 제공 시 masked에서 핀 위치 추출 → refinement 입력 → **100% Recall/Precision**

**실측**: pin_large 20장, pin_synthetic 10장 — Recall 100%, Precision 100%, 학습 3ep ~1.8 min

---

### 10.19 EXE 아티팩트 테스트 이슈 (2026-03-06) — 진행 중
...
| 3 | imgsz 입력창 ROI off 시에도 노출, 입력 시 극심한 렉 | P2 | KeyRelease 시 analyze 제거, ROI off 시 imgsz 640 고정·UI 정리 |

**구현 순서** (2026-03-06 완료):
1. GUI: _update_eta에서 analyze 제거, ETA만 계산. Apply suggested 시에만 백그라운드 analyze ✓
2. GUI: imgsz Spinbox 제거, 640 고정 라벨 (ROI off) ✓
...
2. 1280/640 등 imgsz 제한·강제 제거, 사용자 지정 ROI·박스만 사용

**전략** (상세: `docs/ROI_MANUAL_STRATEGY.md`):

| Phase | 작업 | 목표 | 상태 |
|-------|------|------|------|
| **A** | imgsz cap 제거 | gui, dataset, train에서 1280/640 제한·capped 경고 제거 | ✓ |
...
**구현 내용**:
1. **Phase A**: gui.py imgsz Spinbox 복원(320–4096), capped messagebox 제거, _update_eta_label 추가. dataset.py analyze_dataset imgsz 1280 제한→4096.
...
- Help 탭: ROI Editor 단계, imgsz 320–4096 안내
...
| 이슈 | 조치 | 상태 |
|------|------|------|
| **1. Train 탭 불필요 UI** | Excel 입력 제거 (학습 미사용), imgsz 라벨 "(320–4096)" 간결화 | ✓ |
