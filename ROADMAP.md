...
| # | 증상 | 대응 |
|---|------|------|
| 1 | Train: 2쌍, imgsz 5504, epochs 100 → Building dataset OK, epoch 로그 타이밍에서 렉 후 크래시 | EXE imgsz cap 1920 (OOM 방지), graph poll EXE 비활성화 확인 |
| 2 | 브러쉬로 핀 지정 어려움 | 스퀘어 도구 기본 모드, 8×8 작은 사각형, 핀마다 수십 개 클릭 지정 |
| 3 | 초록 마커가 YOLO 학습 혼동 | 타겟 마스킹 빨간색(255,0,0), annotation RED 우선 |
| 4 | 잘못 지정 마스킹 지우기 없음 | Erase 모드 (우클릭 또는 Erase 버튼) |
| 5 | ROI·타겟 마스킹 YOLO 최우선 | annotation/dataset/inference/roi.py RED 우선 |
| 6 | "Select folder first" 반복 | GUI 시작 시 test_data 자동 채움, 경로 검색 강화 |

**합성 테스트 데이터 경로** (고정·검증 시 필수):

| 경로 | 용도 | 생성 명령 |
|------|------|----------|
| `test_data/pin_synthetic/train/unmasked`, `.../masked` | 기본 (640×480, 40쌍) | `python tools_scripts/generate_pin_test_data.py` |
...
2. GUI 시작 시 test_data 자동 검색·채움 (pin_large_factory 우선)
3. 없으면: 위 생성 명령 실행 후 재시작
